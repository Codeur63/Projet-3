...
def main():
    ...
    applicants = applicants.sort_values("date_demande").drop_duplicates(subset=["applicant_id"], keep="last")

    # Enlever ou supprimer les clients qui sont duplicated
    applicants_ref = applicants[["applicant_id", "date_demande"]]

    # # Fusionner credit avec date_demande pour calculer l'ancienneté du dernier crédit
    credit = credit.merge(applicants_ref[["applicant_id", "date_demande"]], on="applicant_id", how="left")

    # Trier et utiliser les dernieres date de credit
    credit = credit.sort_values(["applicant_id", "date_credit"])

    credit = credit[credit["date_credit"].notna() & credit["date_demande"].notna() & (credit["date_credit"] < credit["date_demande"])]

    credit = credit.merge(partners, on="partenaire_id", how="left")

    mobile_agg = (
        mobile.groupby("applicant_id").agg(
            nb_transaction_mois=("nb_transactions_mois", "mean"), volume_entrant=("volume_entrant_xaf", "mean"), volume_sortant=("volume_sortant_xaf", "mean"), regularite_score=("regularite_score", "mean"), operateur=("operateur", "last"), anciennete_compte_mois=("anciennete_compte_mois", "max")
        )
    ).reset_index()

    # # Aggrégation des crédits
    credit_agg = credit.groupby("applicant_id").agg(
    ...
    credit_agg["taux_defaut_historique"] = credit_agg["nb_credits_defaut_hist"] / credit_agg["nb_credit_safe"]

    credit_agg["taux_remboursement_historique"] = credit_agg["nb_credits_rembourses_hist"] / credit_agg["nb_credit_safe"]
    ...
    df_finascore = pd.merge(applicants, credit_agg, on="applicant_id", how="left")

    # # Fusion avec mobile_money a la suite
    df_finascore = pd.merge(df_finascore, mobile_agg, on="applicant_id", how="left")
    ...
    # Sauvegarde
    df_finascore.to_csv("data/merge/finascore.csv", index=False)
