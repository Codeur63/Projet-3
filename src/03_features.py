...
def main():
    ...
    df_finascore["flag_primo_demandeur"] = (
        df_finascore["historique_credit"].isna().astype(int)
    )    
    # Pas d'historique credit
 
    df_finascore["flag_no_credit_history"] = (
        df_finascore["nb_credits_actifs"].isna() | (df_finascore["nb_credits_actifs"] == 0)
    ).astype(int)
    
    # Manque d'information
    df_finascore["flag_anciennete_emploi_missing"] = (
    ...
    df_finascore["indice_surendettement"] = np.where(
        df_finascore["ratio_endettement"] > 1,
        df_finascore["ratio_endettement"] - 1,
        0
    )
    ...
    df_finascore["revenu_par_credit_actif"] = divide(
        df_finascore["revenu_mensuel_xaf"],
        df_finascore["nb_credits_actifs"] + 1
    )
