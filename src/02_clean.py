...
def main():
    ...
    df_finascore["date_demande"] = pd.to_datetime(df_finascore["date_demande"], errors="coerce")

    df_finascore = df_finascore.sort_values("date_demande").drop_duplicates("applicant_id", keep="last")
    ...
    df_finascore.loc[df_finascore["revenu_mensuel_xaf"] < 0, "revenu_mensuel_xaf"] = np.nan
